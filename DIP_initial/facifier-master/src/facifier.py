...
if __name__ == '__main__':
    ...
    fisher_face_emotion.read(f'C:/Sai/Projects/DIP_initial/facifier-master/src/models/emotion_classifier_model.xml')

    fisher_face_gender = cv2.face.FisherFaceRecognizer_create()
    fisher_face_gender.read(f'C:/Sai/Projects/DIP_initial/facifier-master/src/models/gender_classifier_model.xml')

    # Use model to predict
    choice = input("Use webcam?(y/n) ")
    if (choice == 'y'):
        window_name = "Facifier Webcam (press ESC to exit)"
        start_webcam(fisher_face_emotion, fisher_face_gender, window_size=(350, 350), window_name=window_name, update_time=5)
    elif (choice == 'n'):
        run_loop = True
